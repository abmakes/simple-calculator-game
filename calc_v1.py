...
def reset_program(score):
    ...
    label6.grid(row=5, columnspan=5)

    '''Score keeper'''
    textscore = tk.StringVar()
    textscore.set(score)
    score_label = tk.Label(root, textvariable=textscore)
    score_label.grid(row=0, column=2)
    score_text = tk.Label(root, text="Score:")
    score_text.grid(row=0, column=1)

    '''Other buttons'''

    submitbutton = tk.Button(root, text="SUBMIT", command=correct)
    submitbutton.grid(row=6, columnspan=6)
...
introf = tk.LabelFrame(root, text="Instructions:")
introf.grid(row=1, columnspan=6)

labelintro = tk.Label(introf, text="Select the numbers and operators\nneeded to get the answer")
labelintro.grid(row=1, columnspan=6)
...
labeltimer.grid(row=0, column=4)

labeltext = tk.Label(root, text="Time: ")
